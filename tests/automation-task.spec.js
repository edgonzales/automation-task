...
// ~~~~~~~~ TEST ~~~~~~~~
test('verifyAddToCartWorkflow - Crocs', async ({ page }) => {
...
test('verifyAddToCartWorkflow - Tee, Pant, Tank', async ({ page }) => {
  ...
  await page.getByPlaceholder('Search Up to 40% off Champion').fill(PRODUCT_NAME[1]);
  ...
  for (const paginationItem of await paginationItems.all()) {
    ...
    for (const itemTitle of await itemTitles.all()) {
      const lowerCaseText = await itemTitle.innerText().then(text => text.toLowerCase());
      expect.soft(lowerCaseText).toContain(PRODUCT_NAME[1]);
    }
    await page.waitForLoadState();
    await paginationItem.click();
  }
